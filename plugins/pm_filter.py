    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⬅️ BACK", callback_data=f"pmnext_{req}_{key}_{off_set}"),
             InlineKeyboardButton(f"❄️ PAGE {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}", callback_data="pages")]                                  
        )
...
async def pm_AutoFilter(client, msg, pmspoll=False):    
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"❄️ PAGE 1/{math.ceil(int(total_results) / 6)}", callback_data="pages"),
